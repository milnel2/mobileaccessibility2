...
public class Home extends Activity {
	...
	private void loadLayout() {
		...
		for(int i = 0; i < totalIcons; i++){
			...
			if(c.getString(5).equals("N")){
				external[i] = false;
				images[i] = getResources().getIdentifier(c.getString(2), "drawable", "cs.washington");
			}else{
				external[i] = true;
			}
	...
	public boolean onOptionsItemSelected(MenuItem item) {
	    ...
	    case R.id.menu_places:
			i = new Intent(this, PlaceGrid.class);
			startActivity(i);
	        return true;
	    default:
	...
	public class GridAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			Log.d("Pic2Speech", "width: " + width + " columns " + columns);
			//iv.setMinimumHeight(((View)iv.getParent()).getWidth() / columns);
			currView.setMinimumHeight(width / columns);
			iv.setLayoutParams(new LinearLayout.LayoutParams(width / columns, width / columns));
