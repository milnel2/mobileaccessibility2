...
public abstract class SlideRuleListActivity 
		extends PhoneWandActivity 
	...
	// Time (in millis) to wait before speaking item focus changes.
	private static final long ITEM_SPEAK_WAIT_TIME = 800;
		...
		v.setOnFocusChangeListener(new View.OnFocusChangeListener() {
			...
			public void onFocusChange(View v, boolean hasFocus) {
				if (hasFocus) {//&& ttsIsReady()) {
					if(mScreenCreateTime + ITEM_SPEAK_WAIT_TIME < System.currentTimeMillis()) {
						ttsSpeak(tag, TextToSpeech.QUEUE_FLUSH);
					}
				}
			}
		});
	...
	protected boolean doubleTap(){
		...
		if (mLastLoc == 0) {
			ttsSpeak("Selected " + PREVIOUS, TextToSpeech.QUEUE_FLUSH);
			scrollUp();
		} else if (mLastLoc == mViews.length + 1) {
			ttsSpeak("Selected " + NEXT, TextToSpeech.QUEUE_FLUSH);
			scrollDown();
		} else if (mLastLoc != -1) { // user selects an item
			int listItemIndex = mScrollPosition + mLastLoc - 1;
			
			ttsSpeak("Selected " + mListItems[listItemIndex], TextToSpeech.QUEUE_FLUSH);
...
//		mTts.speak("Selected " + view.getText().toString() + ".", TextToSpeech.QUEUE_FLUSH, utterance);
	...
	private boolean findKey(MotionEvent me) {
		...
		for (int i = 0; i < mViews.length + 2; i++) {
			...
			if (x >= loc[0] && x <= (loc[0] + dim[0]) && y >= loc[1] && y <= (loc[1] + dim[1])) {
				if (view.getTag().equals(BLANK)) {
					// Stop the TTS, so the user knows she has left the bounds of the view.
					mLastLoc = -1;
				} else if (i != mLastLoc) {
					mLastLoc = i;
				}
