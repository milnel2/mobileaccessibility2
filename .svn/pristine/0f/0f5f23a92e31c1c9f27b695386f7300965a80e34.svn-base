       ...
       fromConnection:(AVCaptureConnection *)connection 
{
  ...
  if(*maxx < ((short)width -10)){
    *maxx +=10;
  }
  if(*maxy < ((short)height
              -10)){
    *maxy +=10;
  }
  ...
  if (oneDMode) {
    ...
    //LAURENANDCATIE modified this!
    cropRect.origin.x = *minx;//0.0;
    cropRect.origin.y = *miny;//0.0;
    cropRect.size.width = *maxx-*minx;//CGImageGetWidth(capture);
    cropRect.size.height = *maxy-*miny;//CGImageGetHeight(capture);
    printf("oneDmode %u, %u, %u. %u \n", *minx, *miny, (*maxx-*minx), (*maxy-*miny));
  }
  ...
  cropRect.size.height = *maxy-*miny;//CGImageGetHeight(capture);
  if(count%1000== 0){
    UIImageWriteToSavedPhotosAlbum(scrn, self, nil, nil);
    //printf()
  }
  count ++;
  NSLog(@"decode image %u, %u, %u. %u \n", *minx, *miny, (*maxx-*minx), (*maxy-*miny));
